...
function validate_username($s, $min, $max) {
    if (strlen($s) > $max) { return 2; }
    elseif (strlen($s) < $min) { return 1; }
    else { return 0; }
};

if (validate_username('test', 8, 25) > 0) {
    echo 'Username must be between 8 and 25 characters.';
    echo '<br>';
}
        ...
        function validate_weekday($day) {
            ...
            $options = array(
                ...
                return filter_var($day, FILTER_VALIDATE_INT, $options) !== -1;
        };
